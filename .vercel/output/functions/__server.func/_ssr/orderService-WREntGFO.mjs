...
//#region node_modules/.nitro/vite/services/ssr/assets/orderService-WREntGFO.js
var ORDERS_KEY = "pushpangan_orders_list";
...
var orderService = {
	async createOrder(input) {
		...
		try {
			...
			if (response.ok && data.success) newOrder = data.data;
		} catch (err) {
		...
		if (!newOrder) {
			...
			newOrder = {
				...
				gst: Math.round(input.total_amount * .18),
				...
				estimatedDelivery: input.delivery_date || new Date(Date.now() + 2880 * 60 * 1e3).toISOString().split("T")[0],
		...
		const customerOrders = getStoredOrders().filter((o) => ![
			"ord_1",
			"ord_2",
			"ord_3",
			"PUSH58924",
	...
	async getUserOrders(userId) {
		try {
			const response = await fetch(`${API_BASE}/orders/myorders`, { headers: getAuthHeaders() });
			const data = await response.json();
			if (response.ok && data.success && Array.isArray(data.data)) return data.data;
		} catch (err) {
			console.warn("Backend API getUserOrders failed, falling back to local storage:", err);
		}
		const allOrders = getStoredOrders();
		const cleanedOrders = allOrders.filter((o) => ![
			"ord_1",
			"ord_2",
			"ord_3",
			"PUSH58924",
		...
		if (cleanedOrders.length !== allOrders.length) saveOrders(cleanedOrders);
		if (!userId || userId === "guest" || userId === "all") return cleanedOrders;
		return cleanedOrders.filter((o) => {
			...
			if (o.userId === "guest" || !o.userId) return true;
			return false;
		});
	},
	async getOrderById(id) {
		...
		return getStoredOrders().find((o) => o._id === id || o.id === id || o.orderId === id || o.order_number === id) || null;
	},
	async cancelOrder(id, reason) {
		...
		try {
			...
			if (adminData) {
				...
				const aOrder = adminOrders.find((o) => o._id === id || o.id === id || o.orderNumber === id);
				if (aOrder) {
	...
	async rateOrder(id, productId, rating, comment) {
		try {
	...
	async searchOrders(query) {
		try {
			const response = await fetch(`${API_BASE}/orders/search?query=${encodeURIComponent(query)}`, { headers: getAuthHeaders() });
			const data = await response.json();
			if (response.ok && data.success) return data.data;
		} catch (err) {
		...
		return getStoredOrders().filter((o) => {
			...
			if (o.items?.some((i) => (i.flowerName || i.product_name || "").toLowerCase().includes(q))) return true;
			return false;
		});
	},
	async filterOrders(timeframe, status) {
		try {
			const response = await fetch(`${API_BASE}/orders/filter?timeframe=${encodeURIComponent(timeframe)}&status=${encodeURIComponent(status)}`, { headers: getAuthHeaders() });
			const data = await response.json();
			if (response.ok && data.success) return data.data;
		} catch (err) {
		...
		return getStoredOrders().filter((o) => {
			if (status !== "All" && o.orderStatus !== status && o.order_status !== status) return false;
			return true;
		});
	},
	async getInvoice(id) {
		try {
			const response = await fetch(`${API_BASE}/orders/invoice/${id}`, { headers: getAuthHeaders() });
			const data = await response.json();
			if (response.ok && data.success) return data.data;
		} catch (err) {
