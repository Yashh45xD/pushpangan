  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-3xl border border-border/60 bg-card shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      >
        ...
        {flower.available ? (
          <span style={{ backgroundColor: "#E2DCBE", color: "#4F5535", borderColor: "#9F905E" }} className="absolute right-3 top-3 rounded-full border px-3 py-1 text-[11px] font-extrabold shadow">
            In stock
          </span>
        ) : (
          <span className="absolute right-3 top-3 rounded-full bg-muted px-3 py-1 text-[11px] font-semibold text-muted-foreground shadow">
            Sold out
          </span>
        )}
      </Link>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <button
            ...
            className={`flex-1 rounded-full px-3 py-2 text-center text-sm font-semibold transition ${
              justAdded
                ? "bg-leaf/90 text-white scale-95"
                : flower.available
                ? "bg-accent text-accent-foreground hover:brightness-105"
                : "bg-muted text-muted-foreground cursor-not-allowed opacity-60"
            }`}
          >
            {!flower.available ? "Out of Stock" : justAdded ? "✓ Added!" : "Add to Cart"}
          </button>
        </div>
      </div>
    </article>
